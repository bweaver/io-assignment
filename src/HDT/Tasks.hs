...
import qualified HDT.Interpreters as IOInterpreter        (delay, broadcast, receive, ping, pong,
...
broadcast :: msgVal           -- ^The message to broadcast.
          -> Agent MsgF ()
broadcast msg = liftF' $ (BroadcastF PingPong ())
...
-- best to put these in a Reader monad

runIO :: [Agent MsgF a] -> IO ()
runIO list = do 
  sharedChan   <- atomically $ newTChan
  loggingQueue <- IOInterpreter.loggingQueue

  forM_ list $ (run sharedChan loggingQueue) 
  
run :: SharedChannel -> LoggingQueue -> Agent MsgF a -> IO ()
run sc lq (Pure x) = return ()
run sc lq (Agent op) = interpret sc lq op >>= run sc lq

interpret :: SharedChannel -> LoggingQueue -> MsgF a -> IO a
interpret _ _ (DelayF x) = IOInterpreter.delay >> return x
interpret sc lq (MkPing x) = IOInterpreter.broadcast Ping sc >> return x
interpret sc lq (MkPong x) = IOInterpreter.broadcast Pong sc >> return x
  
interpret sc _ (ReceivePing x) = atomically $IOInterpreter.receive sc >>=  return Ping
interpret sc _ (ReceivePong x) = atomically $IOInterpreter.receive sc >>=  return Pong 
